                    ...
                    <div class="d-grid gap-3 gap-lg-5">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <h4 class="card-title mt-2">@lang("Edit Staff")</h4>
                            </div>

                            <div class="card-body mt-2">
                                <form action="{{ route('admin.role.updateStaff', $singleStaff->id) }}" method="post"
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="role_id"
                                                    ...
                                                    @foreach($roles as $role)
                                                        <option
                                                            value="{{ $role->id }}" {{ $role->id == $singleStaff->role_id ? 'selected' : '' }}>@lang($role->name)</option>
                                                    @endforeach
                                                </select>
                                                @error('role_id')
                                              ...
                                              <span
                                                  class="d-block fs-5">@lang("Display the status of the Staff (active or Inactive)")</span>
                                                    ...
                                                    <input
                                                        ...
                                                        id="status" value="1"
                                                        {{old('status', $singleStaff->status) == '1' ? 'checked':''}}>
                                                </span>
                                                    @error('status')
                                                    <span class="invalid-feedback d-block">{{ $message }}</span>
                                                    @enderror
