...
@section('content')
                ...
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item"><a class="breadcrumb-link"
                                                           href="javascript:void(0)">@lang("Roles And Permissions")</a>
                            </li>
                            <li class="breadcrumb-item active" aria-current="page">@lang("Staff List")</li>
                        </ol>
                    </nav>
                    <h1 class="page-header-title">@lang("All Staffs")</h1>
                </div>

                @if(adminAccessRoute(config('permissionList.Roles_And_Permissions.Manage_Staff.permission.add')))
                    <div class="col-sm-auto">
                        <a class="btn btn-primary" href="{{ route('admin.role.staffCreate') }}">
                            <i class="fas fa-plus-circle me-1"></i> @lang('Create New Staff')
                        </a>
                    </div>
                @endif
            </div>
        ...
        <div class="row">
            <div class="col-sm-6 col-lg-4 mb-3 mb-lg-5">
                <div class="card h-100">
                    <div class="card-body">
                        <h6 class="card-subtitle mb-2">@lang("Total Staff")</h6>
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span
                                    class="js-counter display-4 text-dark">{{$staffRecords['totalStaffs'] ?? 0}}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-sm-6 col-lg-4 mb-3 mb-lg-5">
                <div class="card h-100">
                    <div class="card-body">
                        <h6 class="card-subtitle mb-2">@lang("Active Staffs")</h6>
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span
                                    class="js-counter display-4 text-dark">{{$staffRecords['activeStaffs'] ?? 0}}</span>
                                <span
                                    class="text-body fs-5 ms-1">@lang("From") {{$staffRecords['totalStaffs'] ?? 0}}</span>
                            </div>
                            <div class="col-auto">
                              <span class="badge bg-soft-danger text-danger p-1">
                                <i class="bi-graph-up"></i> {{fractionNumber($staffRecords['activeStaffPercentage']) ?? 0}}%
                              </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-sm-6 col-lg-4 mb-3 mb-lg-5">
                <div class="card h-100">
                    <div class="card-body">
                        <h6 class="card-subtitle mb-2">@lang("Inactive Staffs")</h6>
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span
                                    class="js-counter display-4 text-dark">{{$staffRecords['inActiveStaffs'] ?? 0}}</span>
                                <span
                                    class="text-body fs-5 ms-1">@lang("From") {{$staffRecords['totalStaffs'] ?? 0}}</span>
                            </div>
                            <div class="col-auto">
                              <span class="badge bg-soft-danger text-danger p-1">
                                <i class="bi-graph-up"></i> {{fractionNumber($staffRecords['inActiveStaffPercentage']) ?? 0}}%
                              </span>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <form id="filter_form">
                                                ...
                                                <div class="row">
                                                    <div class="mb-4">
                                                        <span class="text-cap text-body">@lang('Role')</span>
                                                        <div class="row">
                                                            <div class="col-12">
                                                                <input type="text" class="form-control"
    ...
    <div class="modal fade" id="loginAsStaffModal" tabindex="-1" role="dialog" aria-labelledby="loginAsUserModalLabel"
         data-bs-backdrop="static"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="loginAsStaffModalLabel"><i
                            class="bi bi-box-arrow-in-right"></i> @lang('Login As Staff')</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="get" action="" class="loginAccountAction" enctype="multipart/form-data">
                    <div class="modal-body">
                        @lang('Do you want to really login as staff?')
                    </div>
    ...
    <!-- Delete Staff Modal -->
    <div class="modal fade" id="deleteStaffModal" tabindex="-1" role="dialog" aria-labelledby="loginAsUserModalLabel"
         data-bs-backdrop="static"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="deleteStaffModalLabel"><i
                            class="bi bi-box-arrow-in-right"></i> @lang('Delete Confirmation')</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="post" action="" class="deleteStaffForm" enctype="multipart/form-data">
                    @csrf
                    @method('delete')
                    <div class="modal-body">
                        @lang('Do you want delete this staff?')
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-white" data-bs-dismiss="modal">@lang('No')</button>
                        <button type="submit" class="btn btn-primary">@lang('Yes')</button>
    ...
    <script>
        ...
        $(document).on('ready', function () {
            ...
            document.getElementById("filter_button").addEventListener("click", function () {
                ...
                let filterRole = $('#role_filter_input').val();
                let filterStatus = $('#filter_status').val();

                const datatable = HSCore.components.HSDatatables.getItem(0);
                datatable.ajax.url("{{ route('admin.staffSearch') }}" + "?filterUser=" + filterUser + "&filterRole=" + filterRole + "&filterStatus=" + filterStatus).load();
            });
